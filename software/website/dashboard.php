<?php
  /*
  * The dashboard displays the incoming and outgoing connections of the device and functions as the portal
  */
  ...
  // If filtered on incoming connections -> switch
...
<div class="middle-container">
  <div>
    ...
    <form id="filter" action="#filter" class="middle-container">
      <input type="hidden" name="d" value="<?php echo $_GET['d']; ?>">
        <button class="filter-button <?php echo ($outgoing_count + $incoming_count == 0 ? 'left-br' : 'left-tbr'); ?>" title="Outgoing" name="f" value="out" <?php echo $filter_type == 'out' ? 'disabled' : ''; ?>>
          <img src="img/icon_outgoing.png" width="17" height="17" class="button-image"> (<?php echo $outgoing_count; ?>)
        </button>
        <button class="filter-button <?php echo ($outgoing_count + $incoming_count == 0 ? 'right-br' : 'right-tbr'); ?>" title="Incoming" name="f" value="in" <?php echo $filter_type == 'in' ? 'disabled' : ''; ?>>
        ...
        // Display default the outgoing connections, else the displayed connections are based on the filter (in or out)
            foreach($stmt->fetchAll() as $row) {
              echo '<tr>
                      ...
                      echo '<input type="hidden" name="d" value="' . $row[$filter_column] . '">
                          <input type="hidden" name="td" value="' . $row[$display_column] . '">
                        </form>
                      </td>
                      <td>
                        <div class="vertical-gap-5 small-cell">
                          <form action="api.php">
                            <input type="hidden" name="d" value="' . $row[$filter_column] . '">
                            <input type="hidden" name="td" value="' . $row[$display_column] . '">
                            <input type="hidden" name="r" value="/dashboard.php?d=' . $row[$filter_column] . '&f=' . $filter_type . '#filter">';
                            if ($filter_type == 'in') {
